...
m4a는 압축비디오로 무압축 비디오인 wav에 비해 담긴 정보량이 다르다. 그래서 wav로 전체 변경 

멜스펙트로그램에는 기본주파수(F0, 음높이)와 배음(고조파) 패턴이 자연스럽게 나타남
- 기본주파수 (F0) = 음높이 (성대 진동 속도)
- 배음 (Harmonics) = 음색을 결정
- 포먼트 (Formant) = 기본주파수 + 배음 시, 혀·입·코 등 을 지나면서 특정 주파수 대역이 증폭된 것

우리가 느끼는 "높낮이"는 F0가 결정하고, "목소리/악기 고유한 톤"은 배음 분포가 결정"


멜스펙에는 **조음/음소(내용) 정보(포먼트, 스펙트럼 윤곽)**도 들어있어서, 모델이 단어·발음 힌트를 이용할 여지가 있음
